...
from scripts.gui.slider import Slider
...
class Audio_GUI(State):
    def __init__(self, game):
        super().__init__(game, "audio_gui")
        ...
        Label(self.game, [self.buttons], "Audio", (30, 60))

        Label(self.game, [self.buttons], "Master: ", (60, 140), font=Custom_Font.font2)
        self.master_slider = Slider(self.game, [self.buttons], (WIDTH * 0.25, 140), "all")
        Label(self.game, [self.buttons], "BG Music: ", (60, 200), font=Custom_Font.font2)
        Slider(self.game, [self.buttons], (WIDTH * 0.25, 200), "music")
        Label(self.game, [self.buttons], "SFX: ", (60, 260), font=Custom_Font.font2)
        Slider(self.game, [self.buttons], (WIDTH * 0.25, 260), "sfx")
        Label(self.game, [self.buttons], "UI: ", (60, 320), font=Custom_Font.font2)
        Slider(self.game, [self.buttons], (WIDTH * 0.25, 320), "ui")
    ...
    def update(self):
        ...
        if self.master_slider.changed:
            for button in self.buttons:
                if button != self.master_slider and isinstance(button, Slider):
                    button.knob.pos = button.knob.start.lerp(button.knob.end, self.game.music_player.get_pool_volume(button.pool))
                    button.knob.change_volume()
